...
func (details *CurveStableModel) CalcDy() {
	...
	// To test if D is correct. leftSubRight = Left side of the function minus the right side should be 0.
	//
	// left := big.NewInt(1).Add(big.NewInt(1).Mul(big.NewInt(details.A*4), x1ax2), D)
	// right1 := big.NewInt(1).Mul(big.NewInt(details.A*4), D)
	// right2 := big.NewInt(1).Div(big.NewInt(1).Exp(D, big.NewInt(3), nil), big.NewInt(1).Mul(big.NewInt(4), x1mx2))
	// right := big.NewInt(1).Add(right1, right2)
	// leftSubRight := big.NewInt(1).Sub(left, right)
	// _ = leftSubRight
